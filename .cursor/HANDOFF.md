...
## 2026-09-17 현재 (이 창에서 하던 일)

**열린 이슈는 하나다.** 전체공지 이후 로딩·가짜 약관.

- 원인: 현장 전체공지가 `notifications`를 수신자 수만큼 INSERT하고, 행마다 푸시 HTTP를 쏨. 2026-09-17 22:34 UTC GSC 전체공지(378명)가 API를 수 분 동안 504/503으로 만듦. Auth는 프로필이 없으면 약관 미동의로 봄.
- 서버(이미 라이브): 푸시 트리거 `FOR EACH STATEMENT` 1회 HTTP. `dispatch-notification-push` v19. 수신자 최대 8명씩.
- 클라이언트: PR **#464** `cursor/announce-push-batch-consent-6504` (draft, base `main`). 프로필이 오기 전에는 `/consent`로 보내지 않음.
- 약관 플래그는 지워지지 않았음. 웹/앱 가드는 #464 머지 전까지 예전 동작.

이 창의 다른 일(근로자 조재만 계정, 리깅 인양방식 등)은 **새 창에서 다시 꺼내지 말 것.** 필요하면 사용자가 새 이슈로 한 줄만 적는다.
