...
const ApprovalLineManager = forwardRef<ApprovalLineManagerHandle, Props>(function ApprovalLineManager({
  ...
  const handleSave = async (): Promise<boolean> => {
    ...
    if (documentDraft) {
      setSaving(true);
      let stepsForSave = lines;
      if (
        documentDraft.entityType === 'assessment_run'
        && authorUserId
      ) {
        const authorMember = projectMembers.find((m) => m.user_id === authorUserId);
        stepsForSave = forceSubmitterStepToAuthor(lines, {
          userId: authorUserId,
          userName: authorName || authorMember?.display_name || '',
          companyId: authorMember?.company_id || documentDraft.companyId || null,
          companyName: authorMember?.company || '',
        });
        if (stepsForSave !== lines) setLines(stepsForSave);
      }
      const companyId =
      ...
      const { data, error } = await upsertDocumentApprovalDraft({
        ...
        projectId,
        companyId,
        steps: linesToDraftSteps(stepsForSave),
      });
      setSaving(false);
      if (error || !data) {
        toast({
