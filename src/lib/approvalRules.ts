...
/**
 * 자동생성 한 단계 배정.
 * - 시공(상신): 기안자 본인만. 서버가 상신칸 수동 결재를 막으므로 타인(타사 1등 포함) 지정 금지.
...
export type ApproverFilterContextLike = {
  authorCompanyId?: string | null;
  authorCompanyType?: string | null;
  parentGcCompanyId?: string | null;
};
...
/**
 * 시공(상신) 드롭다운: 기안자가 기안 회사 소속이면 직책과 무관하게 맨 앞에 둔다.
 * (상신 단계는 작성자 본인이 자동 완료되어야 함)
 */
export function optionsForApprovalStep(
  ...
  const key = (position || '').toLowerCase();
  const isSupervisor =
    key === 'contractor_supervisor' || key === 'contractor_pic' || key === 'site_supervisor';
  if (isSupervisor && opts?.currentUserId) {
    const me = eligible.find(
      (a) =>
        a.out_user_id === opts.currentUserId
        && (!ctx.authorCompanyId || a.out_company_id === ctx.authorCompanyId)
        && !isWorkerApprover(a),
    );
    if (me && !list.some((a) => a.out_user_id === me.out_user_id)) {
      list = [me, ...list];
    }
  }
  return preferAuthorCompany(list, ctx.authorCompanyId);
}

/** 고정 5단계 자동생성 결과 (UI·테스트 공통). */
export function buildAutoGeneratedApprovalPicks(opts: {
  eligible: EligibleApprover[];
  currentUserId?: string | null;
  authorCompanyId?: string | null;
  authorCompanyType?: string | null;
  parentGcCompanyId?: string | null;
}): Array<{ label: string; position: string; approver: EligibleApprover | undefined }> {
  const ctx: ApproverFilterContextLike = {
    authorCompanyId: opts.authorCompanyId,
    authorCompanyType: opts.authorCompanyType,
    parentGcCompanyId: opts.parentGcCompanyId,
  };
  return FIXED_APPROVAL_STEPS.map((step) => {
    const pool = optionsForApprovalStep(opts.eligible, step.position, ctx, {
      currentUserId: opts.currentUserId,
    });
    return {
      label: step.label,
      position: step.position,
      approver: pickAutoGenerateApprover(step.position, pool, {
        currentUserId: opts.currentUserId,
        authorCompanyId: opts.authorCompanyId,
        parentGcCompanyId: opts.parentGcCompanyId,
      }),
...
/**
 * 클라이언트·서버 공통 결재 권한 규칙 (act_on_entity_approval 과 맞춤).
 * 상신칸(시공)은 상신 RPC가 자동 승인 — 수동 승인/반려 불가.
 */
export function canManuallyActOnApprovalStep(opts: {
  ...
  if (!opts.actorUserId) return { ok: false, reason: 'UNAUTHENTICATED' };
  if ((opts.step.status || '') !== '진행중') return { ok: false, reason: 'NOT_ACTIVE_STEP' };
  if (isSubmitterApprovalStep(opts.step)) return { ok: false, reason: 'SUBMITTER_STEP_NO_SELF_APPROVE' };
  if (opts.priorIncomplete) return { ok: false, reason: 'PRIOR_STEP_NOT_APPROVED' };
  if (opts.step.approver_id && opts.step.approver_id !== opts.actorUserId) {
