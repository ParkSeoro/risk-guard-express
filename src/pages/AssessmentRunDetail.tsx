...
const AssessmentRunDetail = () => {
  ...
  const IMESafeInput = useCallback(({ defaultValue, onCommit, className, autoFocus, ...rest }: {
    defaultValue: string; onCommit: (value: string) => void; className?: string; autoFocus?: boolean;
    ...
    return (
      <Input ref={ref} defaultValue={defaultValue} className={className}
        onCompositionStart={() => { composingRef.current = true; }}
        onCompositionEnd={() => { composingRef.current = false; }}
        onBlur={(e) => { if (!composingRef.current) onCommit(e.target.value); }}
        onKeyDown={(e) => { if (e.key === 'Enter' && !e.nativeEvent.isComposing && !composingRef.current) onCommit((e.target as HTMLInputElement).value); }}
        {...rest}
      />
    );
  }, []);
