...
const TILE_DEFS: Record<MobileTileKey, { icon: any; label: string; sub: string; to: string }> = {
  inspect: { icon: ClipboardCheck, label: "안전점검", sub: "현장 점검 등록", to: "/app/worker/inspect" },
  incident: { icon: AlertOctagon, label: "사고 신고", sub: "아차/경미/중대", to: "/app/worker/incident" },
  tbm: { icon: Users, label: "TBM 진행", sub: "QR 발급/참여", to: "/app/worker/tbm" },
  scan: { icon: ScanLine, label: "QR 스캔", sub: "근로자 출입", to: "/app/worker/scan" },
  permits: { icon: FileCheck2, label: "허가서 결재", sub: "대기 결재", to: "/app/worker/permits" },
  actions: { icon: Wrench, label: "조치 관리", sub: "진행중/완료", to: "/app/worker/actions" },
  alerts: { icon: Bell, label: "알림", sub: "미확인", to: "/app/worker/alerts" },
  approvals: { icon: FileCheck2, label: "전자결재", sub: "결재 대기", to: "/app/worker/approvals" },
