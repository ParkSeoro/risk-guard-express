...
/**
 * Mobile list: approved work plans only → Read-Only Viewer.
 */
export default function MobileWorkPlans() {
  ...
  const load = async () => {
    if (!projectId) return;
    setLoading(true);
    let query: any = (supabase.from("work_plans") as any)
      .select("*")
  ...
  return (
    <div className="min-h-screen bg-muted/30 pb-24">
      <header className="bg-primary text-primary-foreground p-4 flex items-center gap-3 sticky top-0 z-10">
        ...
        <div className="font-bold text-lg flex-1">작업계획 조회</div>
      </header>
      ...
      <main className="p-4 space-y-3 max-w-md mx-auto">
        ...
        <Card className="bg-primary/5 border-primary/30">
          <CardContent className="pt-3 pb-3 text-xs text-muted-foreground flex items-center gap-2">
            ...
            모바일은 승인 완료 작업계획서의 핵심 위험·안전조치 조회만 가능합니다. 작성은 PC에서 하세요.
          </CardContent>
        </Card>
