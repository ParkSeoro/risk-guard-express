  ...
  return (
    <div className="min-h-screen bg-muted/30 p-4">
      <div className="max-w-2xl mx-auto space-y-4">
        ...
        <Tabs defaultValue={openLog ? "exit" : "entry"}>
          <TabsList className="grid grid-cols-4 w-full">
            <TabsTrigger value="education"><FileText className="h-4 w-4 mr-1" />교육</TabsTrigger>
            <TabsTrigger value="risk"><ShieldCheck className="h-4 w-4 mr-1" />위험</TabsTrigger>
            <TabsTrigger value="entry" disabled={!!openLog}><LogIn className="h-4 w-4 mr-1" />입장</TabsTrigger>
            <TabsTrigger value="exit" disabled={!openLog}><LogOut className="h-4 w-4 mr-1" />퇴장</TabsTrigger>
          </TabsList>

          <TabsContent value="education">
            <Card>
              <CardHeader><CardTitle>교육자료 확인</CardTitle></CardHeader>
              <CardContent className="space-y-3">
                {materials.length === 0 ? (
                  ...
                  <div key={m.id} className="border rounded p-3 space-y-2">
                    ...
                    {m.tbm_summary && (
                      <div className="text-xs bg-muted p-2 rounded whitespace-pre-wrap">{m.tbm_summary}</div>
                    )}
                  </div>
                ))}
                <Button onClick={confirmEducation} disabled={eduOk} className="w-full">
                  {eduOk ? <><CheckCircle2 className="h-4 w-4 mr-2" />교육 확인 완료</> : "교육 확인"}
                </Button>
              </CardContent>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="risk">
            <Card>
              <CardHeader><CardTitle>주요 위험요인</CardTitle></CardHeader>
              <CardContent className="space-y-2">
                {materials.flatMap(m => m.key_hazards || []).slice(0, 20).map((h: any, i: number) => (
                ...
                {materials.length === 0 && <div className="text-sm text-muted-foreground">등록된 위험요인이 없습니다.</div>}
              </CardContent>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="entry">
            <Card>
              <CardHeader><CardTitle>입장 등록</CardTitle></CardHeader>
              <CardContent className="space-y-3">
                {activePermit && (
                ...
                <div className="space-y-2">
                  <Label className="flex items-center gap-2"><Checkbox checked={raCheck} onCheckedChange={v => setRaCheck(!!v)} />위험성평가 확인</Label>
                  <Label className="flex items-center gap-2"><Checkbox checked={eduCheck} onCheckedChange={v => setEduCheck(!!v)} />교육 확인</Label>
                  <Label className="flex items-center gap-2"><Checkbox checked={tbmCheck} onCheckedChange={v => setTbmCheck(!!v)} />TBM 참여 확인</Label>
                </div>
                <div>
                  <Label>전자서명</Label>
                  <div className="border rounded bg-white">
                    <SignatureCanvas ref={sigEntry} canvasProps={{ className: "w-full h-32" }} />
                  </div>
                  <Button variant="ghost" size="sm" onClick={() => sigEntry.current?.clear()}>지우기</Button>
                </div>
                <Button className="w-full" onClick={submitEntry} disabled={submitting || !canEnter}>
                  {submitting && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}입장
                </Button>
              </CardContent>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="exit">
            <Card>
              <CardHeader><CardTitle>퇴장 등록</CardTitle></CardHeader>
              <CardContent className="space-y-3">
                <div className="text-sm bg-muted p-2 rounded">
                  입장 시간: {openLog && new Date(openLog.entry_at).toLocaleString("ko-KR")}
                </div>
                <Label className="flex items-center gap-2"><Checkbox checked={noAccident} onCheckedChange={v => setNoAccident(!!v)} /><strong>무재해 확인 (사고 없음)</strong></Label>
                <div>
                  <Label>전자서명</Label>
                  <div className="border rounded bg-white">
                    <SignatureCanvas ref={sigExit} canvasProps={{ className: "w-full h-32" }} />
                  </div>
                  <Button variant="ghost" size="sm" onClick={() => sigExit.current?.clear()}>지우기</Button>
                </div>
                <Button className="w-full" onClick={submitExit} disabled={submitting}>
                  {submitting && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}퇴장
                </Button>
              </CardContent>
            </Card>
          </TabsContent>
