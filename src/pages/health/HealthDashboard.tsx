  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // work_env_measurements / hazard_surveys: project-scoped tables (no company_id)
        const [
          { count: w },
          { count: notC },
          { count: over },
          ...
          workersBase(),
          workersBase().eq("health_checkup_status", "미수검"),
          workersBase().lt("next_checkup_due", new Date().toISOString().slice(0, 10)),
          chemBase(),
          chemBase().eq("is_carcinogen", true),
          supabase
        ...
        setStats({
          workers: w || 0,
          notChecked: notC || 0,
          overdueCheckup: over || 0,
          chemicals: chem || 0,
          carcinogens: carc || 0,
          exceeded: exc || 0,
          surveysActive: surv || 0,
        });
      } catch (e) {
        handle(e, "보건 통계 조회");
      }
  ...
  const cards = [
    {
      ...
      sub: `미수검 / 전체 ${stats?.workers ?? "-"}명`,
      urgent: (stats?.notChecked || 0) > 0,
    },
    {
      to: "/health/checkups",
      icon: Activity,
      title: "기한 초과",
      value: stats?.overdueCheckup ?? "-",
      sub: "차회 진단 기한 경과",
      urgent: (stats?.overdueCheckup || 0) > 0,
    },
    {
      to: "/health/chemicals",
      icon: FlaskConical,
    ...
    },
    {
      ...
      sub: "노출기준 초과 건수",
      urgent: (stats?.exceeded || 0) > 0,
    },
    { to: "/health/education", icon: GraduationCap, title: "보건교육", value: "-", sub: "분기별 이수율" },
    {
      to: "/health/hazard-surveys",
      icon: ClipboardList,
      title: "유해요인조사",
      value: stats?.surveysActive ?? "-",
      sub: "진행 중 설문",
    },
  ];
