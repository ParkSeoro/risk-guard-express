...
const pool: EligibleApprover[] = [
  ...
  mk({ out_user_id: "hse", out_company_id: "sub1", out_company_type: "contractor", out_position: "HSE_MANAGER", out_role: "safety_manager", out_display_name: "협력사안전" }),
...
describe("approval timeline helpers — self-lock / sequential", () => {
  ...
  it("entityTypeLabel covers all approval document types", () => {
    expect(entityTypeLabel("assessment_run")).toBe("위험성평가 · 위평 작성");
    expect(entityTypeLabel("assessment_run_feedback")).toBe("위험성평가 · 이행 확인");
...
describe("pickAutoGenerateApprover", () => {
  ...
  it("시공: author is 기안회사 HSE → still pick author (상신 자동완료)", () => {
    ...
    expect(picks[0].approver?.out_user_id).toBe("hse");
    expect(picks[0].approver?.out_company_id).toBe("sub1");
  });
