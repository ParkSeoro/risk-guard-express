...
describe("notification + entity mobile routes", () => {
  ...
  it("toMobileShellPath keeps worker attendance and signature tabs on mobile", () => {
    expect(toMobileShellPath("/app/admin/workers")).toBe("/app/worker/workers");
    expect(toMobileShellPath("/app/admin/workers?tab=attendance")).toBe(
      "/app/worker/workers?tab=attendance",
    );
