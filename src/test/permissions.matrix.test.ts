...
const EXPECTED: Record<Role, Record<Feature, Perm>> = {
  ...
  // 관리감독자 — 위험성평가 작성 주체 (고시 §7) · 작업계획서 작성 주체 (안전기준규칙 제38조)
  site_supervisor: {
...
describe("권한 매트릭스 회귀 (Role × Feature × Action)", () => {
  ...
  it("site_supervisor(관리감독자)는 작업계획서를 작성할 수 있고 감리(supervisor)는 못 한다", () => {
    expect(EXPECTED.site_supervisor.work_plan.create).toBe(true);
    expect(EXPECTED.site_supervisor.work_plan.edit).toBe(true);
    expect(EXPECTED.supervisor.work_plan.create).toBe(false);
    expect(EXPECTED.supervisor.work_plan.edit).toBe(false);
  });
});
