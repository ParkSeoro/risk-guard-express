...
AS $$
DECLARE _a record; _next record; _now timestamptz := now();
  _all_done boolean; _creator uuid; _entity_label text; _entity_link text;
BEGIN
  ...
  IF _action NOT IN ('approve','reject') THEN RETURN jsonb_build_object('error','INVALID_ACTION'); END IF;
  ...
  IF _action='reject' THEN
    ...
    IF _a.entity_type='work_plan' THEN
      UPDATE public.work_plans SET status='반려', updated_at=_now WHERE id=_a.entity_id RETURNING created_by INTO _creator;
    ...
    ELSIF _a.entity_type='assessment_run' THEN
      BEGIN UPDATE public.assessment_runs SET status='보완중', updated_at=_now
             WHERE id=_a.entity_id RETURNING created_by INTO _creator;
  ...
  SELECT bool_and(status='승인') INTO _all_done FROM public.approvals
   WHERE entity_type=_a.entity_type AND entity_id=_a.entity_id AND approval_version=_a.approval_version;

  IF _all_done THEN
    IF _a.entity_type='work_plan' THEN
      UPDATE public.work_plans SET status='승인', updated_at=_now WHERE id=_a.entity_id RETURNING created_by INTO _creator;
    ELSIF _a.entity_type='work_permit' THEN
      UPDATE public.work_permits SET status='승인', approved_at=_now, approved_by=auth.uid(), updated_at=_now
       WHERE id=_a.entity_id RETURNING created_by INTO _creator;
    ELSIF _a.entity_type='assessment_run' THEN
      BEGIN
        UPDATE public.assessment_runs SET status='승인완료', updated_at=_now
         WHERE id=_a.entity_id RETURNING created_by INTO _creator;
        UPDATE public.risk_items SET is_locked=true WHERE run_id=_a.entity_id;
      EXCEPTION WHEN OTHERS THEN NULL; END;
    END IF;
    IF _creator IS NOT NULL THEN
      INSERT INTO public.notifications (user_id, type, title, body, link)
      VALUES (_creator, 'approval_result', _entity_label||' 최종 승인', '결재가 완료되었습니다.', _entity_link);
    END IF;
  END IF;
