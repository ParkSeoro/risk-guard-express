...
BEGIN
  ...
  SELECT (public.has_role(_caller, 'master'::app_role)
  ...
  -- Backfill/normalize role_new on every existing membership per signup position
    ...
    _mapped := COALESCE(
      ...
      CASE
        ...
        WHEN _mem.position_new::text IN ('inspector','SUPERVISOR')
          THEN 'safety_manager'::project_role
        ELSE 'user'::project_role
      END
    );
    -- Only upgrade; never demote an already-elevated role unless override supplied
    IF _override_role IS NOT NULL
       OR _mem.role_new IS NULL
       OR _mem.role_new = 'viewer'::project_role
       OR _mem.role_new = 'user'::project_role THEN
...
REVOKE ALL ON FUNCTION public.approve_pending_user(uuid, project_role) FROM PUBLIC, anon;
...
DECLARE
  ...
  _is_admin boolean;
BEGIN
  IF _caller IS NULL THEN RAISE EXCEPTION 'AUTH_REQUIRED'; END IF;
  ...
  UPDATE public.profiles SET account_status = 'inactive' WHERE user_id = _user_id;
