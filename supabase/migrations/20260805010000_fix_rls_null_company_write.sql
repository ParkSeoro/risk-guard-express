-- Fix RLS: company_id NULL inserts blocked across modules using can_write_company_data.
-- Symptom: safety_inspections create → "new row violates row-level security policy"
-- Root: UI often omits company_id; can_access/can_write treated NULL as deny.
-- company_id NULL = project-scoped row (readable by members; writable by manager roles).
-- When company_id IS set, GC peer isolation / contractor own-only unchanged.
...
BEGIN
  ...
  -- Project-scoped write (UI often omits company_id): manager roles
  IF _company_id IS NULL THEN
    RETURN _role IN (
      'project_admin', 'safety_manager',
      'site_manager', 'supervisor', 'site_supervisor'
