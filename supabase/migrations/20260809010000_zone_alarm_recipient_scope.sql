-- Zone alarm recipients SSOT:
--   1) Violator (self notify + local GPS siren on device)
--   2) Project admins (project-wide project_admin)
--   3) Managers of the violator's company only
-- Never fan-out to every SM/site_manager on the project.
...
COMMENT ON FUNCTION public.trg_zone_event_notify() IS
  'unauthorized_entry → violator self + project_admin (project-wide) + violator company managers only';
