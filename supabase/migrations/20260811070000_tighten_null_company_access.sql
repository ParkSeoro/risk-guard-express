-- A3: can_access_company_data — company_id NULL must not be public to all members.
-- Evidence:
--   20260805010000 returned true for any project member when _company_id IS NULL
--   (regression vs 20260730100000 / 20260804070000 which denied NULL for gc/contractor).
-- Pattern: work_permits SELECT NULL branch — master (already early-return) or
--   발주처(client/owner) manager roles only.
-- Signature unchanged; can_write_company_data untouched (NULL write already role-gated).
...
BEGIN
  ...
  -- Project-scoped rows (no company): 발주처 관리 역할만 (전 멤버 공개 금지)
  IF _company_id IS NULL THEN
    RETURN _own_type IN ('client', '발주처', 'owner')
      AND _role IN (
