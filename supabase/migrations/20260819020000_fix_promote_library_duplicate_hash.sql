-- Final RA approval (status → 승인완료) promotes risk_items into global_risk_library.
-- Duplicate process/sub_task/hazard rows share content_hash, so
-- INSERT … ON CONFLICT DO UPDATE tried to touch the same row twice:
--   "ON CONFLICT DO UPDATE command cannot affect row a second time"
-- That error aborted the approval trigger (live DB had no EXCEPTION wrapper).
-- Dedupe hashes in the INSERT set, and never let library promote roll back approval.
...
BEGIN
  ...
  SELECT
    ...
    content_hash, source, source_run_id, use_count, quality_score, is_active
  FROM (
    SELECT DISTINCT ON (hashed.content_hash)
      hashed.process_key,
      hashed.process_label,
      hashed.work_phase,
      hashed.sub_task,
      hashed.hazard,
      hashed.hazard_type,
      hashed.hazard_situation,
      hashed.existing_measure,
      hashed.improvement_measure,
      hashed.likelihood_grade,
      hashed.severity_grade,
      hashed.risk_grade,
      hashed.improved_likelihood_grade,
      hashed.improved_severity_grade,
      hashed.improved_risk_grade,
      hashed.frequency,
      hashed.severity,
      hashed.improved_frequency,
      hashed.improved_severity,
      hashed.ppe,
      hashed.legal_basis,
      hashed.equipment_keys,
      hashed.condition_keys,
      hashed.content_hash,
      hashed.source,
      hashed.source_run_id,
      hashed.use_count,
      hashed.quality_score,
      hashed.is_active
    FROM (
      SELECT
        ...
        true AS is_active,
        length(trim(COALESCE(ri.improvement_measure, ''))) AS improvement_len,
        length(trim(COALESCE(ri.hazard_situation, ''))) AS situation_len,
        ri.updated_at
    ...
    ORDER BY
      hashed.content_hash,
      hashed.improvement_len DESC,
      hashed.situation_len DESC,
      hashed.updated_at DESC NULLS LAST
  ) src
