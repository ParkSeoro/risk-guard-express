-- Field announcements must sound like normal approval alerts, not danger sirens.
-- Previously require_ack → severity='high', and dispatch treats severity=high as FCM siren.
...
BEGIN
  ...
  -- severity NULL: same as approval_request — normal default sound, never siren channel.
...
BEGIN
  ...
  -- Keep inbox rows in sync (no re-push). Never raise severity to siren tiers.
...
-- Clear siren-triggering severity on already-sent announcement rows.
UPDATE public.notifications
   SET severity = NULL
 WHERE type = 'announcement'
   AND severity IN ('high', 'critical', 'danger');
