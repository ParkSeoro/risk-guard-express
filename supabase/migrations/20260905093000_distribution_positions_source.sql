-- Distribution map needs to tell check-in GPS from later track-location fixes.
-- OUT columns changed (added source) — must drop before replace.

DROP FUNCTION IF EXISTS public.get_worker_distribution_positions(uuid);
...
COMMENT ON FUNCTION public.get_worker_distribution_positions(uuid) IS
  '오늘 미퇴근 출근자의 최근 GPS (이름·전화 없음). source=checkin|gps 로 출근 점과 추적을 구분.';
