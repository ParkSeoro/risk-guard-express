-- 결재 알림 제목/본문에 entity_type 원문이 그대로 나가던 문제.
-- assessment_run_feedback → 위험성평가 피드백(조치). 기존 17건도 치환.
...
BEGIN
  ...
  _entity_link := CASE _entity_type
    ...
    WHEN 'assessment_run_feedback' THEN '/assessment-run/' || COALESCE(_entity_id::text,'')
    ...
    WHEN 'safety_inspection' THEN '/safety-inspections?id=' || COALESCE(_entity_id::text,'')
    ELSE '/approvals' END;
  ...
  ELSIF _entity_type IN ('assessment_run', 'assessment_run_feedback') THEN
    SELECT COALESCE(NULLIF(period_label,''), NULLIF(type,''))
      INTO _doc_title FROM public.assessment_runs WHERE id = _entity_id;
  END IF;
...
SET
  title = replace(title, 'assessment_run_feedback', '위험성평가 피드백(조치)'),
  message = replace(message, 'assessment_run_feedback', '위험성평가 피드백(조치)'),
  body = replace(body, 'assessment_run_feedback', '위험성평가 피드백(조치)')
WHERE title LIKE '%assessment_run_feedback%'
   OR message LIKE '%assessment_run_feedback%'
   OR coalesce(body, '') LIKE '%assessment_run_feedback%';
