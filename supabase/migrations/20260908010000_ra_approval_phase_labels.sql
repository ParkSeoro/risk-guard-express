-- Phase-explicit RA approval labels: 위평 작성 vs 이행 확인.
...
BEGIN
  ...
  _entity_label := CASE _entity_type
    ...
    WHEN 'assessment_run' THEN '위험성평가 · 위평 작성'
    WHEN 'assessment_run_feedback' THEN '위험성평가 · 이행 확인'
  ...
  ELSIF _entity_type IN ('assessment_run', 'assessment_run_feedback') THEN
    ...
    IF _entity_type = 'assessment_run_feedback' THEN
      _doc_title := COALESCE(_doc_title, '위험성평가') || ' · 이행 확인';
    ELSE
      _doc_title := COALESCE(_doc_title, '위험성평가') || ' · 위평 작성';
    END IF;
  END IF;
