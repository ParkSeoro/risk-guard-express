-- Worker share confirm follows the in-force week (start_date), not the approval instant.
-- Early-approved next-week runs must not notify or accept signatures until that week starts.

CREATE OR REPLACE FUNCTION public.assessment_share_run_is_open(
  _start date,
  _today date DEFAULT ((now() AT TIME ZONE 'Asia/Seoul')::date)
)
RETURNS boolean
LANGUAGE sql
STABLE
...
AS $fn$
  SELECT _start IS NULL OR _start <= _today;
$fn$;

REVOKE ALL ON FUNCTION public.assessment_share_run_is_open(date, date) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.assessment_share_run_is_open(date, date) TO authenticated, service_role;
...
BEGIN
  IF _run.id IS NULL OR _run.status IS DISTINCT FROM '승인완료' THEN
    RETURN 0;
  END IF;
  IF NOT public.assessment_share_run_is_open(_run.start_date) THEN
    RETURN 0;
  END IF;
...
CREATE OR REPLACE FUNCTION public.flush_due_assessment_share_notices(_project_id uuid DEFAULT NULL)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
  ...
  FOR _run IN
    SELECT ar.*
     ...
     WHERE COALESCE(ar.is_deleted, false) = false
       ...
       AND public.assessment_share_run_is_open(ar.start_date, _today)
       AND (ar.end_date IS NULL OR ar.end_date >= _today)
       AND NOT EXISTS (
         SELECT 1 FROM public.notifications n
          WHERE n.type = 'assessment_share'
            AND n.related_id = ar.id::text
       )
  LOOP
    _sent := public.notify_assessment_run_share(_run);
    _n := _n + COALESCE(_sent, 0);
  END LOOP;
  RETURN _n;
END;
$fn$;

REVOKE ALL ON FUNCTION public.flush_due_assessment_share_notices(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.flush_due_assessment_share_notices(uuid) TO authenticated, service_role;
