...
| 영역 | 구현 상태 | 핵심 내용 |
|---|---|---|
| 로컬 운영 UI | 구현 | `127.0.0.1:8787`에서 Gateway·카메라·이벤트·Fleet 상태를 보여 준다. |
...
| systemd | 구현 | 상시 실행·자동 재시작·권한 축소 서비스 정의를 제공한다. |
...
로컬 운영 화면은 `http://127.0.0.1:8787`에서 연다. 개발 모드에서만 Gateway가 암호화 비밀 저장소의 로컬 키를 생성할 수 있다. 운영 환경에서는 `VISION_EDGE_MASTER_KEY`를 systemd credential, TPM, HSM 또는 조직의 비밀 관리 절차로 주입해야 한다.

## 현장 설치 순서

현장 설치 전에는 NVR·카메라 모델, 펌웨어, 코덱, RTSP 읽기 계정, GPU·OS·디스크, 현장 VLAN과 outbound 443 정책을 확정해야 한다. `config/vision-edge.example.json`을 `/etc/safenex-vision-edge/vision-edge.json`으로 복사하고, 모든 certificate 경로와 Fleet endpoint를 기입한다. RTSP URL은 JSON에 넣지 않고 암호화 secret store의 reference로만 연결한다.
